...
// Get team Pokemon from the store
...
<template>
  <main class="min-h-screen team-gradient">
      ...
      <div class="px-4 pb-8">
        ...
        <div v-if="showLoading" class="flex justify-center items-center py-12">
          <div class="text-center">
            ...
            <p class="text-white mt-2">Loading team...</p>
          </div>
        </div>

        <!-- Empty State -->
        <div v-else-if="teamPokemon.length === 0" class="text-center py-12">
          <div class="text-white text-6xl mb-4">⚡</div>
          <h3 class="text-xl font-semibold text-white mb-2">No Team Members Yet</h3>
        ...
        <div v-else>
          <!-- Team Members -->
          <div class="space-y-3">
            <PokemonListItem v-for="pokemon in teamPokemon" :key="pokemon.id" :pokemon="pokemon" />
          </div>
        </div>
      </div>
    </div>
